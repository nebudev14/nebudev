...
export default function About() {
  return (
    <div className="h-full px-16 py-32 mx-40 md:mx-0 md:px-2 md:py-16">
      <h1 className="inline py-2 mb-8 border-b-4 border-pink-600 text-7xl md:text-3xl">Experiences</h1>
      ...
      <div className="grid grid-cols-2 gap-12 md:grid-cols-1">
        {experiences.map((exp, i) => (
          <div
            className="p-4 px-16 duration-200 border border-gray-400 rounded-3xl hover:border-pink-600 md:p-2 md:px-8"
            key={i}
          >
            <h1 className="my-10 text-2xl text-white md:text-lg">{exp.role}</h1>
            <div className="mb-4">
              <Image
                src={`/logos/${exp.img}`}
                alt={exp.name}
            ...
            <h2 className="mb-4 text-4xl text-white md:text-2xl">{exp.name}</h2>
            <h3 className="mb-4 text-2xl text-cyan-400 md:text-xl">{exp.time}</h3>
